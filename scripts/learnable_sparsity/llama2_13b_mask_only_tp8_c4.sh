...
# Task Configs
TAG="llama2-13b-tp8-mask-only-c4-singlenode" # this will be the name of output folder
DATA_INDEX_PATH=CACHE # path to the cache folder. Will generate if not exists
PROJECT_PATH=$(pwd)
OUTPUT_PATH="$PROJECT_PATH/output"

# Transformer Configs
HIDEN_SIZE=5120 # hidden size
NUM_LAYERS=40 # number of layers
NUM_ATTN_HEADS=40 # number of attention heads
SEQ_LENGTH=4096 # sequence length

# Training Configs
TOKENIZER_MODEL="$PROJECT_PATH/assets/checkpoints/llama2_13b_hf/tokenizer.model" # path to the tokenizer model
...
if [ $resume -eq 0 ]; then
    LOAD="$PROJECT_PATH/output/oneshot_pruning/checkpoint/llama2-13b-tp8.sparse.nmprune.sp0.5hessian.ex0" # load the checkpoint
...
--ffn-hidden-size 13824 --normalization RMSNorm \
